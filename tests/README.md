...
| `Discovery.Tests.ps1` | Stages a fake Microsoft plugin `agents/` folder and runs the generator (dry-run) to prove **dynamic worker sub-agent discovery**: one hidden (`user-invocable: false`) wrapper is generated per Microsoft worker, it **points at** Microsoft's file (never forks it), it mirrors the worker's tool intent, it is wired into the owning Lead's `agents:` list, the 12 core agents are untouched, and vanished workers are **pruned**. | Guarantees Leads really gain (and lose) Microsoft's bundled workers on refresh, without hand-authoring or forking. |
