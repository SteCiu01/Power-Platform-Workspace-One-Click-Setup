...
.NOTES
    Requirements: git, node 18+, dotnet (for pac CLI), VS Code with GitHub Copilot
#>
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) { $missing += "node 18+ (https://nodejs.org)" }
...
$copilotSettingsPath = "$rootPath\.github\copilot\settings.json"
if (-not (Test-Path $copilotSettingsPath)) {
    @'
{
...
'@ | Set-Content -Path $copilotSettingsPath -Encoding UTF8
    Write-Host "  Created .github/copilot/settings.json"
}
...
Run:
  cd power-platform-skills && git pull --ff-only && cd ..

If the pull succeeds, confirm:
