...
## IMPORTANT - TOOL WARM-UP AND AUTOMATIC RECOVERY

VS Code tools (especially the terminal) may not be ready immediately when a
workspace is first opened. Before running ANY terminal command, you MUST first
perform a file read operation to let VS Code finish initialising:

1. Read this file: `.github/copilot-instructions.md`
2. Read: `AGENTS.md`
3. Only AFTER both reads succeed, proceed to terminal commands.

### Automatic error recovery

If ANY tool call fails with an internal VS Code error (e.g. "e is not iterable",
"n is not iterable", "Wbt.clearMarks is not a function", "clearMarks is not a function",
or any other tool/extension error):

**STEP 1 - Retry with warm-up:**
- Read one more file (`scripts/pac-workflows.ps1`) to give VS Code more time
- Retry the failed tool call exactly once

**STEP 2 - If the retry ALSO fails, immediately output this exact message and stop:**

---
⚠️ **VS Code tool error detected.**
...
- If your version is **below 1.117.0**: update VS Code from https://code.visualstudio.com and reopen this workspace.
- If your version is **1.117.0 or above**: this is likely a temporary glitch. Try:
  1. Open the **Extensions** sidebar (Ctrl+Shift+X)
  2. Find **GitHub Copilot Chat**
  3. Click the gear icon ⚙️ → **Disable AI Features**
  4. Wait 3 seconds, then click ⚙️ again → **Enable AI Features**
  5. Open a **new chat**, select **Power Platform Master Agent**, and send any message.
---

Do NOT attempt any more tool calls after STEP 2. Wait for the user to act.
When the user reopens chat, start fresh from Phase 0.
...
### Phase 0 - Read the user's first message and warm up tools

**ALWAYS do this first.** Read the user's opening message carefully and classify it:

- **GREETING** = the message is a casual hello, hi, good morning, hey, or similar
  with no specific question or task embedded.
- **QUESTION / TASK** = the message contains an actual question, request, or
  instruction (e.g. "pull my solution", "what environments do I have?",
  "add a field to the Contact form", etc.).

**Save the classification and the original message** - you will need them after setup.

Then silently read these files to let VS Code finish loading (do not print anything):
...
If **GREETING**, greet the user back warmly (match their tone - e.g. "Hey!",
"Good morning!", etc.) then say:
...
If the terminal tool itself errors (internal VS Code error), follow the
**Automatic error recovery** steps defined in the TOOL WARM-UP section above.
...
Then immediately answer or act on the original question/task the user asked
in their first message. Do NOT ask them to repeat it - you saved it in Phase 0.
...
if ($updateMode -or -not (Test-Path $settingsJsonPath)) {
    @'
{
    "task.allowAutomaticTasks": "on"
}
'@ | Set-Content -Path $settingsJsonPath -Encoding UTF8
    Write-Host "  $(if ($updateMode) {'Updated'} else {'Created'}) .vscode/settings.json"
}
